...
| 연결 | 출력 포트 클릭 → 입력 포트 클릭. 리소스가 다르거나 운송 종류(벨트↔파이프)가 다르면 연결 거부 |
| 벨트/파이프 | 원형 포트·회색 실선 = 벨트(30/분), 마름모 포트·청록 점선 = 파이프(120/분). 포트는 설비별 지정 위치(side/pos)에 고정 가능 |
...
| 설비 에디터 | 🛠 설비 편집 — 앱 안에서 설비 추가·수정·삭제·복제. 수정본은 localStorage에 저장되고 JSON으로 내보내 `public/data/facilities.json`을 교체하면 저장소에 반영 |
| 다크모드 | 우상단 🌙/☀️ 토글 |
