...
| 부지 경계 | 부지 프리셋 선택 시(통합 핵심 구역 70×70 — EnKAD 클라이언트 실측) 경계 표시 + 구역 밖 배치 제한. 실게임 규칙 반영: 생산·물류·저장 설비와 벨트는 구역 안 전용, 채집·전력·기능성·전투·기체 설비와 파이프는 구역 밖 가능 |
...
- **부지 크기**: 통합 핵심 구역 70×70은 EnKAD 클라이언트 데이터 실측. 거점별(무릉성/영천대 등) 실제 부지 모양은 구조화된 공개 데이터가 없어 미포함 — `public/data/sites.json`에 프리셋으로 추가 가능
